...
export async function repositionLabelInBrowser(pdfBytes: Uint8Array, corner: Corner) {
  ...
  try {
    const result = await repositionLabelWithDetector(pdfBytes, corner, detectLabelBoundsInBrowser);
    ...
    console.error("[pdf/reposition][browser] repositionLabelInBrowser failed", error);
    throw error;
  }
}
